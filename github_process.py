...
from tu_util import update_node, create_node, create_relationship
...
def process_user(data, token_manager):
    try:
        # 数据已经是反序列化的字典
        ...
        if result:
            update_node("github_user", {"id": result['id']},
...
def process_repo(data, kafka_producer, token_manager):
    ...
    try:
        ...
        # 更新仓库数据
        update_node("github_repo", {"id": result['id']}, {
            "star": result['star'],
        ...
        # 处理关联数据（语言/许可证/主题/组织）
        if result["language"]:
            create_node("language", {"name": result["language"]})
            create_relationship(
        ...
        if result["license"]:
            create_node("license", {"name": result["license"]})
            create_relationship(
                {"label": "github_repo", "properties": {"id": result['id']}},
        ...
        if result["github_organization"]:
            create_node("github_organization", {
        ...
        for star_user in result["starred_users"]:
            if create_node("github_user", {"name": star_user[0], "id": star_user[1]}):
                # 发送到Kafka的user主题
                kafka_producer.send(
                    'github_users',
        ...
        for message in consumer:
            try:
                ...
                if topic == 'github_users':
                    # print(f"[进程 {multiprocessing.current_process().name}] 处理用户任务")
                    process_user(data, token_manager)
